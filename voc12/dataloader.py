...
class VOC12DepthDatasetMSF(VOC12ClassificationDataset):
    ...
    def __getitem__(self, idx):
        ...
        img = imageio.imread(get_img_path(name_str, self.voc12_root))
        depth_img = imageio.imread(os.path.join(self.depth_root, name_str+".png"))
        ...
        out = {"name": name_str, "img": ms_img_list, "size": (img.shape[0], img.shape[1]),
               "label": torch.from_numpy(self.label_list[idx]), "depth": depth_img}
        return out

class VOC12SegmentationDataset(Dataset):

    def __init__(self, img_name_list_path, label_dir, crop_size, voc12_root, depth_root = None,
    ...
    def __getitem__(self, idx):
        ...
        if self.depth_root == None:
            return {'name': name, 'img': img, 'label': label}
        else:
            depth_img = Image.open(os.path.join(self.depth_root, name_str+".png"))
            depth_img = self.resize(depth_img)
            return {'name': name, 'img': img, 'label': label, 'depth': depth_img}

class VOC12AffinityDataset(VOC12SegmentationDataset):
    def __init__(self, img_name_list_path, label_dir, crop_size, voc12_root, depth_root,
                 indices_from, indices_to,
                 rescale=None, img_normal=TorchvisionNormalize(), hor_flip=False, crop_method=None):
        super().__init__(img_name_list_path, label_dir, crop_size, voc12_root, depth_root, rescale, img_normal, hor_flip, crop_method=crop_method)
...
class VOC12DepthClassificationDataset(VOC12ImageDataset):
...
class VOC12DepthAffinityDataset(VOC12SegmentationDataset):
    def __init__(self, img_name_list_path, label_dir, crop_size, voc12_root, depth_root,
                 indices_from, indices_to,
                 rescale=None, img_normal=TorchvisionNormalize(), hor_flip=False, crop_method=None):
        super().__init__(img_name_list_path, label_dir, crop_size, voc12_root, depth_root, rescale, img_normal, hor_flip, crop_method=crop_method)
