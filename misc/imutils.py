...
def get_strided_size(orig_size, stride):
    ...
    # return (torch.div((orig_size[0]-1), stride+1, rounding_mode='trunc'), torch.div((orig_size[1]-1), stride+1, rounding_mode=''))
...
def crop_image(image, crop_index): # numpy image cropping
    h, w = image.shape[:2]
    idx_h = list(range(0, h+1, h//crop_index[0]))
    idx_w = list(range(0, w+1, w//crop_index[1]))
    
    new_images = []
    for i in range(crop_index[0]):
        for j in range(crop_index[1]):
            crop_image = image[idx_h[i]:idx_h[i+1], idx_w[j]:idx_w[j+1]]
            new_images.append(crop_image)
            
    return new_images


def merge_images(image_list, crop_idx):
    h, w = image_list[0].shape[1:3]
    idx_h = list(range(0, h+1, h//crop_idx[0]))
    idx_w = list(range(0, w+1, w//crop_idx[1]))  
    new_image = np.zeros(image_list[0].shape)
    
    for i in range(len(idx_h) - 1):
        for j in range(len(idx_w) - 1):
            img = image_list[i*2 + j]
            img_tr = np.transpose(img, (1, 2, 0))
            resized_img = cv2.resize(img_tr, (w//crop_idx[1], h//crop_idx[0]))
            if len(resized_img.shape) != 3:
                resized_img = np.expand_dims(resized_img, axis = -1)
            new_image[:,idx_h[i]:idx_h[i+1], idx_w[j]:idx_w[j+1]] = HWC_to_CHW(resized_img)
