...
def edge_to_affinity(edge, paths_indices, device):
    ...
    for i in range(len(paths_indices)):
        ...
        paths_indices[i] = paths_indices[i].to(device)
...
def affinity_sparse2dense(affinity_sparse, ind_from, ind_to, n_vertices, device):
    ...
    affinity_dense = torch.sparse.FloatTensor(torch.cat([indices, indices_id, indices_tp], dim=1),
                                       torch.cat([affinity_sparse, torch.ones([n_vertices]), affinity_sparse])).to_dense().to(device)
...
def propagate_to_edge(x, edge, radius=5, beta=10, exp_times=8, device="cuda:0"):
    ...
    sparse_aff = edge_to_affinity(torch.unsqueeze(edge_padded, 0),
                                  path_index.path_indices, device)

    dense_aff = affinity_sparse2dense(sparse_aff, path_index.src_indices,
                                      path_index.dst_indices, ver_padded * hor_padded, device)
