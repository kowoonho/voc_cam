...
def separte_score_by_mask(scores, masks, args):
    instacne_map_expanded = torch.from_numpy(np.expand_dims(masks, 0).astype(np.float32))
    instance_score = torch.unsqueeze(scores, 1) * instacne_map_expanded.to(args.device)
...
def _work(model, dataset, args):
    ...
    with torch.no_grad():
        ...
        for iter, pack in enumerate(tqdm(data_loader)):
            ...
            cams = cam_dict['cam'].to(args.device)
            keys = cam_dict['keys']

            centroids = find_centroids_with_refinement(dp)
            instance_map = cluster_centroids(centroids, dp)
            instance_cam = separte_score_by_mask(cams, instance_map, args)

            rw = indexing.propagate_to_edge(instance_cam, edge, beta=args.beta, exp_times=args.exp_times, radius=5, device=args.device)
